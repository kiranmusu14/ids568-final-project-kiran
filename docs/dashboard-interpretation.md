...
The dashboard layout follows a left-to-right signal hierarchy: infrastructure health (latency, error rate, request count) on the left, and RAG-specific quality signals (retrieval score distribution, TTFT, empty retrieval rate, token throughput) on the right. That ordering surfaces the distinction between serving-layer health and evidence-quality health, which is the core diagnostic split in an agentic RAG system.
